...
public class PostDao {
    ...
    public List<TagPost> getPostForUser(int userId) {
        List<TagPost> list = template.query("SELECT DISTINCT t.id as tagId,t.tag_name as tagName ,p.id as postId,p.title,p.description,p.link,p.count_upvote,p.count_downvote,p.creatorName,p.pubDate,p.rss_link_id" +
    ...
    public List<Post> getPostFrom(String domain) {
        List<Post> list = template.query("SELECT DISTINCT t.id as tagId, t.tag_name as tagName, p.id as postId,p.title,p.description,p.link,p.count_upvote,p.count_downvote,p.creatorName,p.pubDate,p.rss_link_id" +
                " FROM post p JOIN post_tag pt ON p.id = pt.post_id " +
                " JOIN tag t ON t.id = pt.tag_id " +
